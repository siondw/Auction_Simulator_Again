...
class SocketIOManager {
  ...
  sendUserBid() {
    ...
    let currentBid = parseInt(
      CurrentBid_Text.textContent.replace(/[^0-9]/g, "")
    );
    ...
    if (bidAmount > userMaxBid) {
      alert(
        `Your bid cannot be higher than your max bid of $${userMaxBid}.`,
        "error"
      );
      return;
    }
    ...
    this.socket.emit("place_human_bid", {
  ...
  sendNomination(selectedPlayer) {
    this.socket.emit("player_nominated", {
      session_id: this.session_id,
...
function fetchNominationOrder() {
  fetch(
    `${BASE_URL}/t/get-team-names?session_id=${localStorage.getItem(
      "session_id"
    )}`
  )
    .then((response) => response.json())
...
function updateRosterDisplay(rosterData) {
  if (!Array.isArray(rosterData)) {
    console.error(
      "Expected rosterData to be an array but received:",
      rosterData
    );
    return; // Exit the function if rosterData is not an array
  }

  var rosterTable = document
    .getElementById("rosterTable")
    .getElementsByTagName("tbody")[0];
